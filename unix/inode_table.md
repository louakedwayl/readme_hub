...
----------------------
    ...
    Le noyau charge une partie de la table des inodes en mémoire (stocker dans une partie partage de la ram qui s'appele inode cache ) pour accélérer l'accès aux métadonnées des fichiers et répertoires fréquemment utilisés. Seuls les fichiers ouverts sont présents dans l'inode cache.    
